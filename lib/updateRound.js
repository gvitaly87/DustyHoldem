...
const noShowDown = (table, deck) => {
  ...
  deck = tableObj.deck;
  return { table, deck, winnerMessage };
};
...
const updateRound = (table, prevPlayerSeat, deck) => {
  ...
  if (nextPlayer.actionRequired === false /*&& table.round !== 5*/) {
    ...
    roundQue.forEach((seat) => {
      ...
      if (player.chipCount > 0 && !player.allIn) player.actionRequired = true;
      if (player.chipCount <= 0 || player.allIn) player.actionRequired = false;
    });
