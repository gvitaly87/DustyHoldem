...
const updateRound = (table, lastPlaySeat, deck) => {
  const posInQue = table.seatsQue.indexOf(lastPlaySeat);
  const nextPlayerPos = getPos(posInQue + 1, table.seatsQue.length);
  const nextPlayer = table.seats[table.seatsQue[nextPlayerPos]];
